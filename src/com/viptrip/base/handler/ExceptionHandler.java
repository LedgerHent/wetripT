package com.viptrip.base.handler;
...
import com.viptrip.base.exception.BusinessException;
...
public class ExceptionHandler implements HandlerExceptionResolver {
	...
	public ModelAndView resolveException(HttpServletRequest request,
		...
		if (request.getHeader("accept")==null || !(request.getHeader("accept").indexOf("application/json") > -1 || (request
			...
			// 这里需要手动将异常打印出来
			logger.error(StringUtil.getExceptionStr(ex));
			// 对于非ajax请求，都统一跳转到error.jsp页面
			return new ModelAndView("public/exception", map);
		} else {
			// 如果是ajax请求，JSON格式返回
			try {
