...
var date = function ($) {
  ...
  $.fn.hotelDate = function (options) {
    ...
    function initTemplate() {
      ...
      var startWeek;
      var endWeek;
      $('.close-btn').on('click', function (startWeek,endWeek) {
        // 获取入住时间
        ...
        var dateString = enterYear + '-'+enterMonth + '-'+enterDay;
         var dateArray = dateString.split("-");
    	 var date = new Date(dateArray[0], parseInt(dateArray[1] - 1), dateArray[2]);
    	var weeks = new Array("日", "一", "二", "三", "四", "五", "六");
        startWeek = "周" + weeks[date.getDay()];
        $('.week').eq(0).html(startWeek);
        // 获取离开时间
        ...
        var leaveString = leaveYear + '-'+leaveMonth + '-'+leaveDay;
         var leaveArray = leaveString.split("-");
    	 var dates = new Date(leaveArray[0], parseInt(leaveArray[1] - 1), leaveArray[2]);
        var weeksd = new Array("日", "一", "二", "三", "四", "五", "六");
    	endWeek = "周"+weeksd[dates.getDay()];
    	$('.week').eq(1).html(endWeek);
        var night = Number($('.leave').attr('index')) - Number($('.enter').attr('index'));
        if(night > 28){
        	$('.container').MyConfirm({
	            content: "入住和离店时间不能超过28天"
	        });
        	return false;
        }
        if(night == 0){
        	$('.container').MyConfirm({
	            content: "离店日期必须大于入住日期"
	        });
        	return false;
        }
        $('.date').remove(); // 移除插件
        ...
        $('#startDate').val(enterYear + '-' + enterTime);
        $('#endDate').val(leaveYear + '-' + leaveTime);
        //--酒店列表隐藏域--
        $("#checkStartDate").val(enterYear + '-' + enterTime);
        $("#checkOutDate").val(leaveYear + '-' + leaveTime);
        $("#nightCount").val(night);
        //--酒店列表隐藏域--
        $('.few').html(+ night + '晚');
      ...
      $('.day li').on('click', function () {
    	  //过滤空白日期
    	  if(typeof($(this).attr("date-date"))=="undefined"){
    		 return false;
    	  }
    ...
    function initDay() {
      ...
      enterMonth.length === 1 ? enterMonth = '0' + enterMonth : false;
      var enterDay = String(nowdate.getDate()+1);
      enterDay.length === 1 ? enterDay = '0' + enterDay : false;
      var enterTime = enterMonth + '-' + enterDay;
      // 获取离开时间
      ...
      leaveMonth.length === 1 ? leaveMonth = '0' + leaveMonth : false;
      var leaveDay = String(nowdate.getDate() + 2);
      leaveDay.length === 1 ? leaveDay = '0' + leaveDay : false;
      var leaveTime = leaveMonth + '-' + leaveDay;
      if($('.entertime').text()==""&& $('.leavetime').text()==""){
    	  $('.entertime').text(enterTime); // 显示
          $('.leavetime').text(leaveTime);
      }
      $('#startDate').val(enterYear + '-' + enterTime);
