...
<html>
	...
	<head>
        ...
        <form id="subForm" action="flight/getFlightList.act" method="post">
         	...
         	<input type="hidden" id="flowId1"  name="flowId" value='${flowId[0]}' />
			<input type="hidden" id="depAirport1" name="depAirport" value='${depAirport[0]}' />
			<input type="hidden" id="arrAirport1" name="arrAirport" value='${arrAirport[0]}' />
			<input type="hidden" id="date1" name="date" value='${date[0]}' />
			<input type="hidden" id="flowId2"  name="flowId" value='${flowId[1]}' />
			<input type="hidden" id="depAirport2" name="depAirport" value='${depAirport[1]}' />
			<input type="hidden" id="arrAirport2" name="arrAirport" value='${arrAirport[1]}' />
			<input type="hidden" id="date2" name="date" value='${date[1]}' />
            ...
            <input type="hidden" id="selectusers" name="selectusers" value='${selectusers }' /> <!-- 差旅规则选中的乘机人 -->
	    </form>
	<body onpagehide="$.fn.LoadingHide()">
		...
		<section>
			<header>
				...
				<p class="zhixiang">添加乘机人</p>
			    <div id="selectpassersOk"  style="width:5rem; text-align:center;color:#00afec;background:#ffffff;border-radius:0.5rem;margin:auto 0;height:2rem;line-height:2rem;">
					确定
				</div> 
			</header>
			<div class="table-content">
				...
				<ul class="passener-content pas-table">
					...
					<li class="left-passener" style="border: 0px solid red;">
						<div class="left-passener" style="border: 0px solid red;">
							<div class="middle-table" style="border: 0px solid red;">
								<div class="passener-name">
								    ...
								    <input type="hidden" value="${staff.departmentId}"><%--乘机人部门--%>
								    <input type="checkbox" id="certificate${status.count}" /> ${staff.name }
								</div>
	...
	<script type="text/javascript">
		...
		$('.left-passener').each(function(i){
			$("#certificate"+(i+1)).on('click',function(){
				var checkedstr=$(this)[0].checked;
			    var selectusersval = $("#selectusers").val();
			    //alert("selectusersvalold:"+selectusersval);
				//selectusersval = $("#selectusers").val();//最新值 
				selectusers(selectusersval,checkedstr,$(this));
				//selectusersval = $("#selectusers").val();//最新值 
				//$.cookie("selectusersval",selectusersval);
			    //alert("selectusersvalold:"+selectusersval);
			});
		});
	    /*------选择完乘机人后页面跳转------*/
	    ...
	    $("#selectpassersOk").on('click',function(){
	    	var selectusersval = $("#selectusers").val();//最新值
	    	$("input[id^='certificate']").each(function(i){
				var checkedstr=$("#certificate"+(i+1))[0].checked;
				selectusers(selectusersval,checkedstr,$(this));
				selectusersval = $("#selectusers").val();//最新值
			}); 
	    	
	    	if(selectusersval==""){
	    	   alert("企业已开启差旅管控,至少需要选择一个乘机人。");
	    	   return false;
	    	}else{
    			$("#subForm").attr("action","flight/getFlightList.act");
	    ...
	    function selectusers(selectusersval,checkedstr,selectObj){
	    		selectusersval = $("#selectusers").val();
	    		if(checkedstr){
			    	var id=selectObj.parent().children().val();
				    var name=selectObj.parent().children().next().val();
				    var mobile=selectObj.parent().children().next().next().val();
				    var userIdType=selectObj.parent().children().next().next().next().val();
				    var userId=selectObj.parent().children().next().next().next().next().val();
				    var passengerType=selectObj.parent().children().next().next().next().next().next().val();
				    var departId=selectObj.parent().children().next().next().next().next().next().next().val();
				    //110101198711094550_294274_6360_6360_管理审核员（测试）
				    //alert(id+"__"+name+"__"+mobile+"__"+userIdType+"__"+userId+"__"+passengerType);
				    var name2=userIdType==2?name+"(VIP)":name;
				    var selectusersstr=(userId==""?"0123456789":userId)+"_"+id+"_"+departId+"_"+departId+"_"+name2;//departId人员所在部门的ID
				    //alert("selectusersstrnew:"+selectusersstr);
			    	if(selectusersval.indexOf(selectusersstr) >= 0){
			    	   //alert("包含");
			    	}else{
			    		if(selectusersval!=""){
			    			$("#selectusers").val(selectusersval+"|"+selectusersstr);
			    		}else{
				    		$("#selectusers").val(selectusersstr);
			    		}
			    	}
				}else{
					var userslist=selectusersval.split("|");
					var newSelectusersval="";
					for(var i=0;i<userslist.length;i++){
						//alert(userslist[i]);
						if(userslist[i]!="" && userslist[i]!=selectusersstr){
						    if(newSelectusersval!=""){
						    	newSelectusersval+="|";
						    }
							newSelectusersval+=userslist[i];
						}
					}
					//alert("取消选择:"+newSelectusersval);
					$("#selectusers").val(newSelectusersval);
				}
	    }
